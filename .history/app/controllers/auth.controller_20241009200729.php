...
class AuthController
{
    ...
    {
        if (empty($_POST['dni'])) {
            return $this->view->showInicioSesion('Falta completar el dni');
        }

        if (empty($_POST['contrasena'])) {
            return $this->view->showInicioSesion('Falta completar la contraseña');
        }

        $dni = $_POST['dni'];
        $contrasena = $_POST['contrasena'];
        $usuarioDB = $this->model->getUsuarioBydni($dni);
