...
require_once '../tpe/app/modelos/config.php';
define('BASE_URL', '');
...
switch ($parametros[0]) {
    ...
    case 'eliminarVenta':
        ...
        $controlador = new ControladorVentas($res);
        $controlador->borrarVenta($parametros[1]);
        $controlador->mostrarVentas();
        break;
    case 'agregarVenta':
        AutSesionMiddleware($res);
    ...
    case 'editarVenta':
        ...
        VerificarAutMiddleware($res);
        $controlador = new ControladorVentas($res);
        $controlador->editarVenta($parametros[1]);
        break;
    ...
    case 'venta':
        ...
        if (!empty($parametros[1])) {
            $controlador = new ControladorVentas($res);
            $controlador->mostrarVenta($parametros[1]);
        }
        break;
    case 'clientes':
        AutSesionMiddleware($res);
    ...
    case 'eliminarCliente':
        ...
        VerificarAutMiddleware($res);
        $controlador = new ControladorClientes($res);
        $controlador->borrarCliente($parametros[1]);
        $controlador->mostrarClientes();
        break;
    case 'editarCliente':
        AutSesionMiddleware($res);
        VerificarAutMiddleware($res);
        $controlador = new ControladorClientes($res);
        $controlador->editarCliente($parametros[1]);
        break;
    case 'cliente':
        if (!empty($parametros[1])) {
            $controlador = new ControladorClientes($res);
