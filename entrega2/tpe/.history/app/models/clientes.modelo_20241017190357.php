...
class Clientes extends Modelo
{
    ...
    public function obtenerVentasCliente($id_cliente)
    {
        $query = $this->db->prepare('
        SELECT ventas.*
        FROM ventas
        INNER JOIN clientes ON ventas.clave_foranea = clientes.id_cliente
        WHERE clientes.id_cliente = ?');
        $query->execute([$id_cliente]);
        return $query->fetchAll(PDO::FETCH_OBJ);
    }

    public function insertarCliente($dni, $nombre, $telefono, $mail, $fecha_nacimiento, $imagen)
    {
    ...
    public function editarCliente($id_cliente, $nombre, $telefono, $mail, $fecha_nacimiento, $imagen)
    {
        $query = $this->db->prepare('UPDATE clientes SET nombre = ?, telefono = ?, mail = ?, fecha_nacimiento = ?, imagen = ? WHERE id_cliente = ?');


        $query->execute([$nombre, $telefono, $mail, $fecha_nacimiento, $imagen, $id_cliente]);
    }
}
